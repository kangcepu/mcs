...
class WoProductionCreateController extends GetxController {
  ...
  Future<void> loadMasterDataThenUserDivision() async {
    ...
    if (woTypes.isNotEmpty && selectedTypeWo.value.isEmpty) {
      selectedTypeWo.value = woTypes[0]['code'];
    }

    if (priorities.isNotEmpty && selectedPriority.value.isEmpty) {
      selectedPriority.value = priorities[0]['code'];
    }
  ...
  Future<void> submitWo() async {
    ...
    try {
      ...
      await _woProductionRepository.createWo(formData);

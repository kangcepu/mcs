...
class WoListController extends GetxController
  ...
  void _startRealtimeUpdates() {
    ...
    _realtimeTimer = Timer.periodic(_realtimeInterval, (_) async {
      if (!Get.isRegistered<WoListController>()) {
        return;
      }
  ...
  Future<void> checkPermissions() async {
    try {
      ...
      print(
          'WO IT Permissions - View: ${canViewWo.value}, Create: ${canCreateWo.value}');
    } catch (e) {
      ...
      if (!canViewWo.value) {
        if (showError) {
          _showWarningSnackbar(
              'Anda tidak memiliki izin untuk melihat Work Order IT');
        }
        return;
      }
  ...
  List<WorkOrder> _buildBaseFilteredList() {
    ...
    List<WorkOrder> filtered =
        sourceList.where((wo) => !_shouldHideFromList(wo.status)).toList();
  ...
  void goToDetail(WorkOrder wo) async {
    final result = await Get.toNamed('/wo/detail', arguments: wo.woNumber);
  ...
  void goToCreate() {
    ...
    Get.toNamed('/wo/create')?.then((result) {
      if (result == true) {
        refresh();
      }
    });

...
class WoGaRepository {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == true) {
          ...
          throw Exception(data['message'] ?? 'Failed to load WO GA detail');
        }
      } else {
        throw Exception('Failed to load WO GA detail: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Get WO GA detail error: $e');
    }
  }

  Future<String> generateWoNumber() async {
    try {
      final response = await _apiService.get(ApiConstants.getNewWoGa);
  ...
  Future<Map<String, dynamic>> approveWo(
    ...
    try {
      final response = await _apiService.post(ApiConstants.approveWoGa,
  ...
  Future<Map<String, dynamic>> addJobExplanation(
    ...
    try {
      ...
      final response = await _apiService.post(ApiConstants.addJobExplanationWoGa,
  ...
  Future<Map<String, dynamic>> completeWo(
    ...
    try {
      final response = await _apiService.post(ApiConstants.completeWoGa,
  ...
  Future<Map<String, dynamic>> closeWo(String woNumber, String comment) async {
    try {
      final response = await _apiService.post(ApiConstants.closeWoGa,
          data: {'wo_number': woNumber, 'comment': comment});
      if (response.statusCode == 200) {
        final data = response.data;
        if (data['status'] == true) return data;
  ...
  Future<Map<String, dynamic>> voidWo(String woNumber, String reason) async {
    try {
      final response = await _apiService.post(ApiConstants.voidWoGa,

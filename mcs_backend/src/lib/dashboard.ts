...
export async function getDashboard(userInput: User, filters: DashboardFilters): Promise<Record<string, unknown>> {
  ...
  for (const [key, cfg] of selected) {
    ...
    const statusRows = await rows<{ s: string; c: number }>(
    ...
    const prevRow = await one<{ pt: number | null; pc: number | null }>(
    ...
    const companyRows = await rows<{ co: string; c: number }>(
    ...
    const assetRows = await rows<{ eq: string; c: number }>(
    ...
    const createdRows = await rows<{ d: string; c: number }>(
    ...
    const closedRows = await rows<{ d: string | null; c: number }>(
    ...
    const agingRow = await one<{ b0: number | null; b1: number | null; b2: number | null; b3: number | null }>(
      `SELECT

...
async function insertApprovalIt(woNumber: string, person: ReturnType<typeof personPayload>, comment: string): Promise<void> {
  await execute(
...
async function generateWoNumber(divisionCode: string): Promise<string> {
  const now = new Date();
  const base = `WOIT-${String(now.getMonth() + 1).padStart(2, '0')}${now.getFullYear()}/${divisionCode}`;
  const row = await one<{ seq: number | null }>('SELECT MAX(CAST(RIGHT(wo_number,4) AS UNSIGNED)) AS seq FROM tb_wo_it WHERE wo_number LIKE ?', [`${base}/%`]);
...
async function getHeader(woNumber: string): Promise<Record<string, unknown> | null> {
  return one<Record<string, unknown>>('SELECT * FROM tb_wo_it WHERE wo_number=?', [woNumber]);
}
...
isRouter.use('/is', authenticate, (req, res, next) => {
  ...
  const crossAccess = Number(user.wo_cross_access ?? 0) === 1;
  const nativeAccess = Number(user.wo_it ?? 0) === 1;
  if (crossAccess && !nativeAccess) throw new HttpError(403, 'Cross WO Access is read-only');
  next();
});

// Samakan sama daftar exclude di maintenance/meso — sebelumnya cuma exclude
// 'CLOSED', jadi WO VOID/REJECT/COMPLETE_EXECUTOR nyampah di list "aktif".
const LIST_EXCLUDED_STATUSES = ['CLOSED', 'COMPLETE', 'DONE', 'COMPLETE_EXECUTOR', 'COMPLETE EXECUTOR', 'NEED_CLOSED', 'VOID', 'DECLINE', 'REJECT'];

isRouter.get('/is/list', asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
...
isRouter.get('/is/my_wo', asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
  const page = Math.max(1, Number(req.query.page ?? 1));
  const limit = Math.min(500, Math.max(1, Number(req.query.limit ?? 20)));

  const total = await one<{ total: number }>("SELECT COUNT(*) total FROM tb_wo_it WHERE status != 'CLOSED' AND creator=?", [user.fullname]);
  const items = await rows<Record<string, unknown>>(
    `SELECT w.*, d.division_name, d.division_code, a.AssetID, a.AssetName FROM tb_wo_it w
...
isRouter.get('/is/detail', asyncHandler(async (req, res) => {
  ...
  const header = await one<Record<string, unknown>>(
    `SELECT w.*, d.division_name, d.division_code, a.AssetID, a.AssetCode, a.AssetName FROM tb_wo_it w
...
isRouter.get('/is/dashboard', asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
  ...
  const [openCount, progressCount, closedCount] = await Promise.all([
    one<{ total: number }>(`SELECT COUNT(*) total FROM tb_wo_it ${open.sql}`, open.params),
    one<{ total: number }>(`SELECT COUNT(*) total FROM tb_wo_it ${progress.sql}`, progress.params),
    one<{ total: number }>(`SELECT COUNT(*) total FROM tb_wo_it ${closed.sql}`, closed.params),
  ]);
...
isRouter.get('/is/get_new', asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
...
isRouter.get('/is/test_user', asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
...
isRouter.post('/is/create', attachmentUpload.array('attachment', 10), asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
  const b = req.body;
  ...
  await transaction(async (connection) => {
    await connection.execute(
      `INSERT INTO tb_wo_it (wo_number, date, company, shift, type_wo, priority, id_division, id_equipment, job_title, running_hours, job_requirement, attachment, job_executor, status, pic, creator, created_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())`,
      [woNumber, b.date, b.company, b.shift ?? 'REGULAR', b.type_wo, b.priority ?? 'NORMAL', b.id_division, idEquipment, b.job_title,
        b.running_hours ?? '', b.job_requirement ?? '', attachment, 'ITS', 'WAIT_KA_DIV_ITIS', 'ITS', user.fullname ?? 'API User'] as never,
    );
    await connection.execute('INSERT INTO tb_job_executor (job_executor, wo_number, job_explanation, status, created_at) VALUES (?,?,?,?,NOW())', ['ITS', woNumber, '', 'WAITING']);
  });
...
async function updateHandler(req: AuthRequest, res: import('express').Response): Promise<void> {
  ...
  await transaction(async (connection) => {
    ...
    await connection.execute(`UPDATE tb_wo_it SET ${keys.map((k) => `\`${k}\`=?`).join(',')}, updated_at=NOW() WHERE wo_number=?`, [...keys.map((k) => fields[k]), woNumber] as never);
  });
...
async function approveTransition(woNumber: string, person: ReturnType<typeof personPayload>, position: string, callerIsIts: boolean, comment: string): Promise<string> {
  ...
  let pic: string;
  if (position === 'DIVHEAD' && callerIsIts) {
    targetStatus = 'WAIT_EXECUTOR_ADMIN';
    pic = String(header.job_executor ?? 'ITS');
    ...
    targetStatus = 'CLOSED';
    pic = '-';
  } else if (position === 'ADMIN_DIVISI' && callerIsIts) {
  ...
  await transaction(async (connection) => {
    ...
    if (targetStatus === 'CLOSED') {
      ...
      await connection.execute('UPDATE tb_wo_it SET status=?, pic=?, updated_at=NOW() WHERE wo_number=?', [targetStatus, pic, woNumber]);
    }
  });
...
isRouter.post('/is/add_job_explanation', servicePhotoUpload.array('service_photos', 10), asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
  const b = req.body;
  ...
  const header = await getHeader(finalWoNumber);
...
isRouter.post('/is/add_labor', asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
  const b = req.body;
...
isRouter.post('/is/complete', asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
  const woNumber = String(req.body.wo_number ?? '');
  ...
  await transaction(async (connection) => {
    await connection.execute('INSERT INTO tb_approval_it (wo_number, fullname, avatar, id_division, id_position, comment, created_at) VALUES (?,?,?,?,?,?,NOW())',
      [woNumber, person.fullname, person.avatar, person.id_division, person.id_position, comment]);
    await connection.execute("UPDATE tb_wo_it SET started_actual=?, finished_actual=?, status='NEED_CLOSED', updated_at=NOW() WHERE wo_number=?",
...
isRouter.post('/is/closed', asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
  const woNumber = String(req.body.wo_number ?? '');
...
isRouter.post('/is/void', asyncHandler(async (req, res) => {
  const user = (req as AuthRequest).user!;
  ...
  await transaction(async (connection) => {
    await connection.execute("UPDATE tb_wo_it SET status='VOID', reason=?, pic='-', updated_at=NOW() WHERE wo_number=?", [reason, woNumber]);
    await connection.execute('INSERT INTO tb_approval_it (wo_number, fullname, avatar, id_division, id_position, comment, created_at) VALUES (?,?,?,?,?,?,NOW())',
